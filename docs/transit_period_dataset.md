...
```bat
python -m astrology_graph_foundry.cli transit --provider live --target-dataset alex_natal_dataset.json --start 2026-01-01 --end 2027-07-01 --ephe-path C:\dev\swisseph --out alex_transit.json
```

For a single day:

```bat
python -m astrology_graph_foundry.cli transit --provider live --target-dataset alex_natal_dataset.json --date 2026-01-01 --ephe-path C:\dev\swisseph --out alex_2026-01-01_transit.json
```
